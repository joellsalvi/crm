(function (angular, undefined) {
	...
	var module = angular.module('asCustomSelect', []);

	module.value('customSelectDefaults', {
		displayText: 'Selecione...',
		emptyListText: 'Não existem registros para a pesquisa realizada.',
		emptySearchResultText: 'Nenhum registro com "$0"',
		addText: 'Adicionar',
		searchDelay: 1000
	});
			...
			link: function (scope, elem, attrs, controller) {
				...
				var itemTemplate = elem.html().trim() || '{{' + (match[2] || match[1]) + '}}',
					...
					'<a class="dropdown-toggle" data-toggle="dropdown" href ng-class="{ disabled: disabled }">' +
						...
						'<b></b>' +
					'</a>' +
					'<div class="dropdown-menu" style="max-width:508px">' +
						'<div stop-propagation="click" class="custom-select-search">' +
							'<input class="' + attrs.selectClass + '" style="max-width:508px" maxlength="100" type="text" autocomplete="off" ng-model="' + searchModel + '" />' +
						'</div>' +
						'<ul role="menu">' +
